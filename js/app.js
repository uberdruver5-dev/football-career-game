...
class FootballApp {
  ...
  socialPost() {
    ...
    const likes=Math.round((8000+Math.random()*18000)*tier*luck*Math.pow(reach,.42)); const gained=Math.round((300+Math.random()*2600)*tier*luck*Math.pow(reach,.30));
  ...
  updateSocialAfterMatch(goals, assists, won, compKey, roleActions=0) {
    ...
    // Actual generated players retain their own follower totals and change after every matchday.
    Object.values(window.leaguesEngine.clubSquads||{}).forEach(sq=>sq.forEach(pl=>{const baseline=this.socialPlayerBaseline(pl);pl.socialFollowers = (pl.socialFollowers || 0) < 10000000 ? baseline : Math.max(pl.socialFollowers || 0, baseline);pl.socialFollowers=Math.max(1000,pl.socialFollowers+Math.round((pl.socialFollowers||0)>20000000 ? ((Math.random()-.42)*(1000000+Math.random()*7000000)) : ((Math.random()-.42)*(pl.ovr||70)*180)));}));
  }
    ...
    container.innerHTML = currentContract + offers.map(o => `
      <div class="glass-panel" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        ...
        <div style="text-align: right;">
          ...
          <div style="font-size: 12px; color: var(--accent-gold);">${p.position === 'GK' ? '+$5,000/Save' : (['CB','LB','RB','CM'].includes(p.position) ? '+$5,000/Defensive Action' : `+$${o.goalBonus}/Goal`)} • +$${o.assistBonus}/Assist</div>
    ...
    clubs.forEach((c, idx) => {
      ...
      squad.forEach(pl => {
        ...
        if (compKey === 'season') {
          ...
          compMatches = (compKey.startsWith('national_') || ['world_cup','euro','copa','international'].some(k => compKey.includes(k))) ? Math.max(1, window.leaguesEngine.natGroupPlayed || 3) : Math.max(1, Math.round(currentMatches * ratio));
        ...
        // League matchdays are global: every club player has played the same number of league games.
        const plMatches = (compKey === 'league' || compKey === 'season') ? (compKey === 'season' ? (s.matches || currentMatches) : currentMatches) : ((pl.stats && (pl.stats[key]?.matches || 0)) || compMatches);
  ...
  simulateMatch() {
    ...
    if (['ST', 'LW', 'RW', 'CAM'].includes(pos)) {
      ...
      const shots = Math.floor(Math.random() * (shotCap + 1));
    ...
    if (['CB', 'LB', 'RB', 'CM'].includes(pos) || pos === 'GK') {
      ...
      const roleActionCount = pos === 'GK' ? (Math.floor(Math.random() * 15) + 1) : (Math.floor(Math.random() * 14) + 2);
  ...
  _processSimEvent(ev) {
    ...
    if (ev.type === 'goal') {
      ...
      sim.motmScores[sim.userName]=(sim.motmScores[sim.userName]||0)+0.5;sim.motmTeams[sim.userName]=sim.userTeam; this._refreshMotm();
      this._updateLiveSimPlayerStats();
  ...
  _finishSim() {
    ...
    squad.forEach(tm => {
      ...
      else if (tm.position === 'GK') tm.goalkeeperSaves = (tm.goalkeeperSaves || 0) + Math.floor(Math.random() * 15) + 1;
      else if (['ST','LW','RW','CAM'].includes(tm.position)) tm.shotsOnGoal = (tm.shotsOnGoal || 0) + Math.floor(Math.random() * 12);
    });
