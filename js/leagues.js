...
class LeaguesEngine {
  ...
  getCulturalCountry(countryName) {
    ...
    // 7. Arabic culture. Several Arabic national teams do not have their own
    // name table yet, so they deliberately share the Arabic/Egyptian table — never Turkey.
    if (['egypt', 'saudi arabia', 'algeria', 'tunisia', 'iraq', 'qatar', 'uae',
         'united arab emirates', 'jordan', 'oman', 'morocco', 'libya', 'palestine',
         'lebanon', 'syria', 'yemen', 'bahrain', 'kuwait', 'sudan'].some(country => c.includes(country))) {
      return 'Egypt';
    }
    // Turkish names are reserved exclusively for Turkey.
    if (c.includes('turkey')) return 'Turkey';
    // South Asian fallback group.
    if (['uzbekistan', 'india', 'bangladesh', 'pakistan'].some(country => c.includes(country))) {
      return 'India';
    }
  ...
  distributeMatchStatsToSquad(clubId, goalsScored) {
    ...
    // Goalkeeper save model for AI-simulated matches: saves = shots - goals,
    // matching the user's own matches (keeper faces 6-28 shots per game).
    const teamShots = Math.floor(Math.random() * 23) + 6;
    const gkSaves = Math.max(0, teamShots - (goalsScored || 0));
    ...
    squad.forEach(tm => {
      ...
      if (tm.position === 'GK') tm.goalkeeperSaves = (tm.goalkeeperSaves || 0) + gkSaves;
    });
    ...
    for (let i = 0; i < goalsScored; i++) {
      ...
      // 70% of goals receive an assist. Wingers/CAMs drive the creative split,
      // with a modest OVR bonus rather than an overwhelming one.
      if (Math.random() < 0.70) {
        const assistWeights = { LW: 35, RW: 35, CAM: 20, LM: 20, RM: 20, CM: 5, ST: 3, LB: 2, RB: 2, CB: 1, GK: 0.2 };
        const candidates = squad.filter(pl => pl.name !== (scorer ? scorer.name : ''));
        const total = candidates.reduce((sum, pl) => sum + (assistWeights[pl.position] || 2) * (1 + Math.max(-0.10, Math.min(0.15, ((pl.ovr || 70) - 70) * 0.005))), 0);
        let roll = Math.random() * total;
        const assister = candidates.find(pl => (roll -= (assistWeights[pl.position] || 2) * (1 + Math.max(-0.10, Math.min(0.15, ((pl.ovr || 70) - 70) * 0.005)))) <= 0) || candidates[0];
        if (assister) {
          if (!assister.stats) {
            assister.stats = {
              season: { goals: 0, assists: 0, matches: 0 },
  ...
  generateSquad(clubId, avgOvr) {
    ...
    const arabicCountries = ['Egypt', 'Saudi Arabia', 'Algeria', 'Tunisia', 'Morocco', 'Qatar', 'UAE', 'Jordan', 'Oman'];
    const isArabicClub = !isNationalTeam && ['saudi_d1', 'saudi_d2'].includes(this.findLeagueForClub(clubId));
    const isTurkishClub = !isNationalTeam && ['turkey_d1', 'turkey_d2'].includes(this.findLeagueForClub(clubId));
    ...
    return positions.map((pos, idx) => {
      ...
      if (!isNationalTeam) {
        ...
        if (tier === 1) {
          ...
          if (!isLocal) {
            const otherPool = countriesPool.filter(c => c !== country && (!isArabicClub || arabicCountries.includes(c)) && (!isTurkishClub || c === 'Turkey'));
  ...
  reorderSeasonSchedule() {
    // HARD RULE: every club game (league, UCL, domestic cup) comes before every
    // national-team match (qualifiers first, then finals tournaments). Late-season
    // insertions (UCL semi-finals, cup finals, national knockouts) are sorted into
    // their group by priority so club fixtures never interrupt the national segment.
    const group = (e) => {
      const key = e.competitionKey || '';
      if (key.startsWith('national_')) return key.includes('qual') ? 1 : 2;
      return 0;
    };
    this.seasonSchedule.sort((a, b) => group(a) - group(b) || a.sortKey - b.sortKey);
    ...
    if (currentFix.competitionKey !== 'league') {
      ...
      if (currentFix.competitionKey === 'ucl') {
        if (currentFix.stageLabel === 'Group Stage') {
          ...
          if (userAgg === oppAgg) {
            // The live simulator supplies the actual shootout winner for an aggregate tie.
            userWins = userMatchScore?.wonOnPenalties === true ? true : (userMatchScore?.lostOnPenalties === true ? false : Math.random() < 0.5);
          }
          ...
          if (won) {
            if (window.userCareer?.profile?.trophies) {
              ...
              window.userCareer.awardFirstTrophySkill('continental', 3);
        ...
        if (won) {
          if (currentFix.stageLabel === 'Round of 16') {
            ...
            if (window.userCareer?.profile?.trophies) {
              ...
              window.userCareer.awardFirstTrophySkill('cup', 2);
      ...
      if (currentFix.competitionKey === 'national_final') {
        const compName = currentFix.competitionName;
        if (won) {
          let goldenBootNotice = '';
          const internationalBoot = window.userCareer?.checkGoldenBoot('international');
          if (internationalBoot?.won && window.userCareer?.profile?.trophies) {
            const profile = window.userCareer.profile;
            profile.trophies.golden_boot = (profile.trophies.golden_boot || 0) + 1;
            if (!profile.awardsCabinet.goldenBoots) profile.awardsCabinet.goldenBoots = { league: 0, cup: 0, ucl: 0, international: 0 };
            profile.awardsCabinet.goldenBoots.international = (profile.awardsCabinet.goldenBoots.international || 0) + 1;
            goldenBootNotice = `<br><br>👟 <strong>${compName.toUpperCase()} GOLDEN BOOT!</strong> You finished as the tournament's top scorer with <strong>${internationalBoot.userGoals} goals</strong>.`;
          }
          if (window.userCareer?.profile?.trophies) {
            if (compName === 'World Cup') {
              window.userCareer.profile.trophies.world_cup = (window.userCareer.profile.trophies.world_cup || 0) + 1;
              window.userCareer.awardFirstTrophySkill('world_cup', 4);
              ...
              window.userCareer.awardFirstTrophySkill('euros_copas', 3);
              if (!window.userCareer.profile.awardsCabinet.internationalCups) {
                window.userCareer.profile.awardsCabinet.internationalCups = [];
              }
          ...
          this.pendingUclNotice = {
            ...
            body: `HISTORIC TRIUMPH! Your country has defeated ${oppName} (<strong>${uGoals} - ${oGoals}</strong>) in the Grand Final! You lift the prestigious ${compName} trophy and achieve eternal glory!${goldenBootNotice}`
          };
        } else {
          this.pendingUclNotice = {
            success: false,
  ...
  evaluateContractOffers(userOvr, currentClubId) {
      ...
      if (currentClub) {
        ...
        if (currentLeague && currentLeague.tier === 1) {
          ...
          if (userOvr < threshold) {
            baseWage = Math.floor(Math.random() * 120001) + 100000; // Div 1 Bench: 20k - 30k
            role = "Bench Player";
          } else {
            baseWage = Math.floor(Math.random() * 180001) + 160000; // Div 1 Starter: 50k - 100k
            role = "First Team Regular";
          }
        } else if (currentLeague && currentLeague.tier === 2) {
          if (userOvr < 72) {
            baseWage = Math.floor(Math.random() * 14000) + 1000; // Div 2 Bench: 2k - 7k
            role = "Bench Player";
          } else {
            baseWage = Math.floor(Math.random() * 45000) + 10000; // Div 2 Starter: 15k - 24k
            role = "First Team Regular";
          }
        }
        
        const rawWage = Math.round(baseWage * (0.75 + (currentClub.stars || 2) * 0.12) * (1 + Math.max(-0.08, Math.min(0.12, (userOvr - 70) * 0.002))) * mult * (0.92 + Math.random() * 0.16));
        // Renewal wage caps vary by star rating (raised so renewals are not all capped
        // at the same 450K ceiling) and still keep Division 2 contracts grounded.
        const wage = currentLeague.tier === 2 ? Math.min(rawWage, role === 'Bench Player' ? 30000 : 85000) : Math.min(rawWage, currentClub.stars <= 2 ? 160000 : (currentClub.stars === 3 ? 320000 : (currentClub.stars === 4 ? 580000 : 820000)));
        ...
        offers.push({
          ...
          wage: wage,
          goalBonus: Math.round(wage * 1.25),
          assistBonus: Math.round(wage * 0.75),
          years: years,
          squadRole: role,
          roleBonus: 5000,
          isRenewal: true
    ...
    selected.forEach(({ club, league }) => {
      ...
      if (league.tier === 1) {
        ...
        if (userOvr < threshold) {
          wageBase = Math.floor(Math.random() * 120001) + 100000; // Div 1 Bench: 20k - 30k
          role = "Bench Player";
          goalMultiplier = 9;
          assistMultiplier = 6;
        } else {
          wageBase = Math.floor(Math.random() * 180001) + 160000; // Div 1 Starter: 50k - 100k
          role = "First Team Regular";
          goalMultiplier = 15;
          assistMultiplier = 10;
        }
        ...
        if (userOvr < 72) {
          wageBase = Math.floor(Math.random() * 14000) + 1000; // Div 2 Bench: 2k - 7k
          role = "Bench Player";
          goalMultiplier = 6;
          assistMultiplier = 4;
        } else {
          wageBase = Math.floor(Math.random() * 45000) + 10000; // Div 2 Starter: 15k - 24k
          role = "First Team Regular";
          goalMultiplier = 12;
          assistMultiplier = 8;
        }
      ...
      const mult = window.userCareer ? window.userCareer.getSalaryMultiplier() : 1.0;
      const rawWage = Math.round(wageBase * (0.75 + (club.stars || 2) * 0.12) * (1 + Math.max(-0.08, Math.min(0.12, (userOvr - 70) * 0.002))) * mult);
      // Keep Division 2 contracts grounded even when reputation multipliers are active.
      const wage = league.tier === 2 ? Math.min(rawWage, role === 'Bench Player' ? 30000 : 85000) : Math.min(rawWage, club.stars <= 2 ? 140000 : (club.stars === 3 ? 250000 : (club.stars === 4 ? 500000 : 750000)));
      const years = Math.floor(Math.random() * 4) + 1; // Beautiful spread of 1 to 4 year contracts!
      ...
      offers.push({
        ...
        logo: club.logo,
        wage: wage,
        goalBonus: Math.round(wage * (0.9 + Math.random() * 0.6)),
        assistBonus: Math.round(wage * (0.4 + Math.random() * 0.5)),
        years: years,
    ...
    // Elite club offers: once you are above 79 OVR, the European superclubs can
    // approach you even without trophies on your shelf. If you are not yet at
    // their starter threshold you receive a BENCH contract (bench wage + bench
    // role) — the price of joining a superclub early. Reach their threshold and
    // they offer you a premium Crucial Star deal instead.
    const eliteClubs = [
      ...
      { id: "psg", name: "Paris Saint-Germain", logo: "🗼", stars: 5, ovr: 85, leagueName: "Ligue 1 (Div 1)" },
      { id: "liverpool", name: "Liverpool FC", logo: "🔴", stars: 5, ovr: 84, leagueName: "Premier League (Div 1)" },
      { id: "arsenal", name: "Arsenal FC", logo: "🔴⚪", stars: 5, ovr: 84, leagueName: "Premier League (Div 1)" },
      { id: "inter", name: "Inter Milan", logo: "🔵⚫", stars: 5, ovr: 84, leagueName: "Serie A (Div 1)" },
      { id: "juventus", name: "Juventus", logo: "⚪⚫", stars: 5, ovr: 83, leagueName: "Serie A (Div 1)" },
      { id: "dortmund", name: "Borussia Dortmund", logo: "🟡⚫", stars: 4, ovr: 83, leagueName: "Bundesliga (Div 1)" },
      { id: "atletico", name: "Atlético Madrid", logo: "🔴⚪", stars: 5, ovr: 83, leagueName: "La Liga (Div 1)" }
    ];
    ...
    if (userOvr > 79) {
      eliteClubs.forEach(club => {
        if (club.id !== currentClubId && !offers.some(o => o.clubId === club.id)) {
          ...
          let wage, goalBonus, assistBonus, squadRole;

          if (userOvr >= threshold) {
            // Premium star-level contract at the superclub.
            const wageBase = club.ovr * 350;
            wage = Math.round((wageBase + Math.random() * 2000) * mult);
            goalBonus = Math.round(club.ovr * 20 * mult);
            assistBonus = Math.round(club.ovr * 15 * mult);
            squadRole = 'Crucial Star';
          } else {
            // Bench contract at a superclub: Div-1 bench wages (20k-30k/wk scaled
            // by stars & reputation), real bench role in-game (sub-ins, etc.).
            const wageBase = Math.floor(Math.random() * 120001) + 100000;
            wage = Math.round(wageBase * (0.75 + club.stars * 0.12) * mult);
            goalBonus = Math.round(wage * 1.25);
            assistBonus = Math.round(wage * 0.75);
            squadRole = 'Bench Player';
          }
    ...
    offers.push(...eliteOffers);

    // Shuffle to mix renewal with external offers
    offers.sort(() => Math.random() - 0.5);
    let finalOffers = offers.slice(0, 5);
    // Guarantee elite-club offers (Real Madrid, City, Barca...) are not shuffled
    // out entirely: always keep at least one visible when available.
    if (eliteOffers.length > 0 && !finalOffers.some(o => o.isElite)) {
      finalOffers.pop();
      finalOffers.push(eliteOffers[Math.floor(Math.random() * eliteOffers.length)]);
    }
    return finalOffers;
  }
  ...
  getClubBadgeHtml(clubName, size = 28) {
    ...
    if (nat) {
      return this.getCountryFlagHtml(nat.name, size);
    }
  ...
  getCountryFlagHtml(countryNameOrCode, width = 24) {
    ...
    // Use the game’s own emoji flags instead of a remote image. This is reliable offline,
    // avoids failed image alt text (such as “BO”), and covers every national team.
    const value = String(countryNameOrCode);
    const specialFlags = { Scotland: 'gb-sct', Wales: 'gb-wls' };
    if (specialFlags[value]) return `<img src="assets/flags/${specialFlags[value]}.svg" width="${width}" height="${Math.round(width * 0.75)}" style="vertical-align:middle;border-radius:3px;object-fit:cover" alt="${value}">`;
    const nation = this.nationalTeams.find(n => n.name === value || n.id === value || n.flag === value);
    if (nation) {
      // Convert the stored regional-indicator emoji to ISO-3166 and load the bundled SVG.
      // Bundling avoids unsupported flag emoji rendering as text (for example “BO”).
      const points = [...nation.flag].map(char => char.codePointAt(0));
      if (points.length === 2) {
        const iso = String.fromCharCode(points[0] - 0x1F1E6 + 65, points[1] - 0x1F1E6 + 65).toLowerCase();
        return `<img src="assets/flags/${iso}.svg" width="${width}" height="${Math.round(width * 0.75)}" style="vertical-align:middle;border-radius:3px;object-fit:cover" alt="${nation.name}">`;
      }
      return `<span style="font-size:${width}px;vertical-align:middle;line-height:1">${nation.flag}</span>`;
    }
    return '⚽';
  }
}
