...
class LeaguesEngine {
  ...
  distributeMatchStatsToSquad(clubId, goalsScored) {
    ...
    squad.forEach(tm => {
      ...
      if (tm.position === 'GK') tm.goalkeeperSaves = (tm.goalkeeperSaves || 0) + Math.floor(Math.random() * 15) + 1;
    });
  ...
  reorderSeasonSchedule() {
    // Finals tournaments are the season finale; club league/UCL/cup fixtures complete first.
    const clubEntries = this.seasonSchedule.filter(e => !e.competitionKey?.startsWith('national_'));
    const nationalFinals = this.seasonSchedule.filter(e => e.competitionKey?.startsWith('national_') && !e.competitionKey.includes('qual'));
    const nationalQualifiers = this.seasonSchedule.filter(e => e.competitionKey?.startsWith('national_') && e.competitionKey.includes('qual'));
    this.seasonSchedule = [...clubEntries, ...nationalQualifiers, ...nationalFinals];
  ...
  evaluateContractOffers(userOvr, currentClubId) {
    ...
    if (Math.random() < 0.67) {
      ...
      if (currentClub) {
        ...
        const rawWage = Math.round(baseWage * (0.75 + (currentClub.stars || 2) * 0.12) * (1 + Math.max(-0.08, Math.min(0.12, (userOvr - 70) * 0.002))) * mult);
        const wage = currentLeague.tier === 2 ? Math.min(rawWage, role === 'Bench Player' ? 30000 : 85000) : Math.min(rawWage, currentClub.stars <= 2 ? 140000 : (currentClub.stars === 3 ? 220000 : 450000));
