...
class PlayerCareer {
  ...
  calculateOvr() {
    ...
    this.profile.ovr = Math.min(maxCeiling, Math.max(1, Math.round(ovr)));
  ...
  getSalaryMultiplier() {
    ...
    if (leagueTitlesCount > 0) {
      mult *= Math.pow(1.15, leagueTitlesCount);
    }
  ...
  upgradeAttribute(attrName) {
    ...
    if (!this.profile.attributes[attrName] || this.profile.attributes[attrName] >= 130) return false;
  ...
  awardFirstTrophySkill(trophyKey, points) {
    if (!this.profile.awardSkillClaims) this.profile.awardSkillClaims = {};
    if (this.profile.awardSkillClaims[trophyKey]) return false;
    this.profile.awardSkillClaims[trophyKey] = true;
    this.profile.skillPoints += points;
    return true;
  }

  recordMatchPerformance(goals, assists, passAccuracy, rating, compKey = "league", didPlay = true, matchOutcome = 'draw', oppGoals = 0, roleActionCount = null) {
    ...
    // Role-specific progression stats. They are generated only for defensive/central
    // midfield roles so position peers can be compared without inflating attackers.
    if (['CB', 'LB', 'RB', 'CM'].includes(p.position)) {
      const actions = roleActionCount ?? (Math.floor(Math.random() * 14) + 2); // 2–15 per match
      p.defensiveActions = (p.defensiveActions || 0) + actions;
      s.defensiveActions = (s.defensiveActions || 0) + actions;
      const oldMilestone = Math.floor((p.defensiveActions - actions) / 75);
      const newMilestone = Math.floor(p.defensiveActions / 75);
      if (newMilestone > oldMilestone) p.skillPoints += newMilestone - oldMilestone;
    } else if (p.position === 'GK') {
      const saves = roleActionCount ?? (Math.floor(Math.random() * 15) + 1); // 1–15 per match
      p.goalkeeperSaves = (p.goalkeeperSaves || 0) + saves;
      s.goalkeeperSaves = (s.goalkeeperSaves || 0) + saves;
      // Every 150 career saves = 1 skill point (requested rebalance for the new
      // shots-minus-goals save model, which produces much higher save counts).
      const oldMilestone = Math.floor((p.goalkeeperSaves - saves) / 150);
      const newMilestone = Math.floor(p.goalkeeperSaves / 150);
      if (newMilestone > oldMilestone) p.skillPoints += newMilestone - oldMilestone;
    }
    ...
    // Qualifiers count toward overall season/career totals, not the finals-event tables.
    const isQualifier = (key.endsWith('_qual') && key !== 'cup_qual') || key.includes('qualifier');
    // Competition keys include domestic, continental, and international tournaments.
    // Create a bucket for any valid new key so those results are never silently lost.
    if (!isQualifier) {
      if (!s.compStats[key]) s.compStats[key] = { goals: 0, assists: 0, matches: 0 };
    ...
    const oldMilestones = Math.floor(oldGoals / 10);
    const newMilestones = Math.floor(c.totalGoals / 10);
      ...
      if (window.app) {
        setTimeout(() => {
          window.app.showGameNotice(
            ...
            `<div style="font-size:54px;margin-bottom:12px;">🎯</div><p style="font-size:16px;color:#fff;font-weight:700;">Career Milestones Reached!</p><p style="font-size:14px;color:#cbd5e1;line-height:1.6;">You have scored an amazing total of <strong>${c.totalGoals} career goals</strong>! Every 10 goals awards a skill point. You earned <strong>+${spEarned} Skill Points</strong>! Spend them in the Skills tab to upgrade your player!</p>`
          );
        }, 1200);
      }
    }

    const rolePayment = ['CB', 'LB', 'RB', 'CM', 'GK'].includes(p.position) ? ((roleActionCount || 0) * (p.roleBonus || 5000)) : 0;
    const matchMoney = p.weeklyWage + (goals * p.goalBonus) + (assists * p.assistBonus) + rolePayment;
  ...
  addXp(amount) {
    ...
    while (this.profile.xp >= this.profile.xpToNextLevel) {
      ...
      this.profile.skillPoints += 2; // +2 skill points per level up!
  ...
  checkGoldenBoot(compKey) {
    ...
    // User goals
    const userGoals = compKey === 'international'
      ? Object.entries(s.compStats || {}).filter(([key]) => key.startsWith('national_')).reduce((sum, [, stat]) => sum + (stat.goals || 0), 0)
      : ((s.compStats && s.compStats[compKey] && s.compStats[compKey].goals) || 0);
    ...
    if (userGoals >= maxAiGoals && userGoals > 0) {
  ...
  checkBallonDorWinner() {
    ...
    // Evaluate user score — identical scoring language to the AI superstars so a
    // high OVR alone can never beat players with better seasons. OVR, goals and
    // rating must all contribute before the user can lead the race.
    let userScore = 0;
    if (userMeetsCriteria) {
      userScore += Math.max(0, (p.ovr - 80) * 2.0);
      ...
      // Trophies can outweigh a small OVR/goals gap, especially when the player contributed.
      userScore += (p.trophies?.league_d1 || 0) * 6;
      userScore += (p.trophies?.cup || 0) * 10;
      userScore += (p.trophies?.continental || 0) * 16;
      userScore += (p.trophies?.euros_copas || 0) * 20;
      userScore += (p.trophies?.world_cup || 0) * 26;
    }
    ...
    allBigClubs.forEach(cl => {
      ...
      // STRICTER CRITERIA: Candidates must have OVR >= 88!
  ...
  advanceSeason() {
    ...
    if (this.pendingTransfer) {
      ...
      if (pt.roleBonus) this.profile.roleBonus = pt.roleBonus; // negotiated role bonus ($/action or $/save)
      if (pt.squadRole) this.profile.squadRole = pt.squadRole; // bench contracts from big clubs stay bench until promoted
    ...
    // 5.5 Social world reshuffles every season: the top-50 celebrity list is
    // re-generated with a fresh random seed so follower counts/rankings change
    // between seasons (the player's own followers are never reset).
    this.profile.socialTopInitialized = false;
    this.profile.socialSeed = Math.floor(Math.random() * 1000000) + 1;
  ...
  exportSaveData() {
    ...
    // Fixtures and schedules reference standing objects, which in turn contain full squads.
    // Saving those references duplicated every squad hundreds of times and could exceed the
    // browser's localStorage quota after a refresh/new career. Squads are saved once below.
    const withoutSquadReferences = (value) => JSON.parse(JSON.stringify(value || [], (key, item) => (
      key === 'squad' ? undefined : item
    )));
    return {
      profile: this.profile,
      stats: this.stats,
      standings: withoutSquadReferences(le.standings),
      fixtures: withoutSquadReferences(le.fixtures),
      seasonSchedule: withoutSquadReferences(le.seasonSchedule),
      ...
      natGroupStandings: le.natGroupStandings,
  ...
  importSaveData(data) {
    ...
    if (data.profile) {
      ...
      this.profile.name = String(this.profile.name || "Jack Sterling").replace(/[<>&"'`]/g, '').trim().slice(0, 40) || "Jack Sterling";
    }
    ...
    if (le) {
      ...
      // Saved standings deliberately omit embedded squad copies to keep saves small.
      // Reconnect each row to its single canonical squad after loading.
      if (le.standings && le.clubSquads) {
        le.standings.forEach(row => {
          row.squad = le.clubSquads[row.clubId] || [];
        });
      }
      ...
      if (data.natGroupStandings !== undefined) le.natGroupStandings = data.natGroupStandings;
