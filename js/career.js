...
class PlayerCareer {
  ...
  recordMatchPerformance(goals, assists, passAccuracy, rating, compKey = "league", didPlay = true, matchOutcome = 'draw', oppGoals = 0, roleActionCount = null) {
    ...
    if (['CB', 'LB', 'RB', 'CM'].includes(p.position)) {
      ...
      const oldMilestone = Math.floor((p.goalkeeperSaves - saves) / 75);
      const newMilestone = Math.floor(p.goalkeeperSaves / 75);
      if (newMilestone > oldMilestone) p.skillPoints += newMilestone - oldMilestone;
    }
    ...
    const rolePayment = ['CB', 'LB', 'RB', 'CM', 'GK'].includes(p.position) ? ((roleActionCount || 0) * 5000) : 0;
  ...
  checkBallonDorWinner() {
    ...
    if (userMeetsCriteria) {
      ...
      userScore += (p.trophies?.league_d1 || 0) * 8;
      userScore += (p.trophies?.cup || 0) * 14;
      userScore += (p.trophies?.continental || 0) * 22;
      userScore += (p.trophies?.euros_copas || 0) * 26;
      userScore += (p.trophies?.world_cup || 0) * 34;
    }
