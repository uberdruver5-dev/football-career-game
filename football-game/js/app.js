...
class FootballApp {
  ...
  randomizePlayerCreation() {
    ...
    // 3. Random Name fitting nationality from window.NATIONAL_NAMES
    let first = "Alex";
    let last = "Hunter";
    if (window.NATIONAL_NAMES && window.NATIONAL_NAMES[randNation.name]) {
      const firstNames = window.NATIONAL_NAMES[randNation.name].first;
      const lastNames = window.NATIONAL_NAMES[randNation.name].last;
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        ...
        if (isDiv1 && p.ovr < 82) {
          ...
          welcomeNotice = {
            ...
            body: `You joined the Division 1 club <strong>${p.currentClubName}</strong>, but because your OVR is under 82, you start on the bench! Your starting bench wage is <strong>$${p.weeklyWage.toLocaleString()}/wk</strong>. Train hard to reach <strong>82+ OVR</strong> and promote to the starting lineup with massive salaries!`
          };
      ...
      if (natSelect) {
        natSelect.addEventListener('change', (e) => {
          ...
          if (window.NATIONAL_NAMES && window.NATIONAL_NAMES[selectedNat]) {
            const firstNames = window.NATIONAL_NAMES[selectedNat].first;
            const lastNames = window.NATIONAL_NAMES[selectedNat].last;
  ...
  renderSquad() {
    ...
    if (isNational) {
      ...
      currentClub = window.leaguesEngine.standings.find(cl => cl.clubId === selectedClubId);
      if (!currentClub) {
        const dbClub = window.leaguesEngine.findClubById(selectedClubId);
        if (dbClub) {
          currentClub = {
            clubId: dbClub.id,
            name: dbClub.name,
            squad: window.leaguesEngine.clubSquads[dbClub.id] || []
          };
        }
      }
    }
  ...
  renderTrophies() {
    ...
    const catalog = [
      ...
      },
      {
        ...
        desc: 'Conquer Europe club competition with a 5-Star club.',
        icon: '⭐',
        color: '#00d2ff'
      },
      {
  ...
  renderMultipliers() {
    ...
    const wageEl = document.getElementById('val-estimated-wage');
    if (!container || !totalEl || !wageEl) return;
    ...
    wageEl.innerText = `$${Math.round(20000 * mult).toLocaleString()} / week`;
  ...
  simulateMatch() {
    ...
    const homeSquad = ((ctx.home && ctx.home.squad) || this._getClubSquad(ctx.home) || []).filter(pl => pl.position !== 'GK');
    const awaySquad = ((ctx.away && ctx.away.squad) || this._getClubSquad(ctx.away) || []).filter(pl => pl.position !== 'GK');
    ...
    if (pos === 'CAM' || pos === 'CM') {
      ...
      assistChance = 0.02; // rare assist from goalkeeper long kicks
    } else {
    ...
    for (let i = 0; i < userGoals; i++) {
      ...
      // If the player is a defender (CB, LB, RB), most (80%) of their goals should be dead-balls (penalties or freekicks)
      let isPenalty = Math.random() < 0.15;
      let isFreekick = !isPenalty && Math.random() < 0.12;

      if (isUserScorer && (pos === 'CB' || pos === 'LB' || pos === 'RB')) {
        if (Math.random() < 0.80) {
          // Force dead-ball goal!
          if (Math.random() < 0.5) {
  ...
  _getClubSquad(clubInput) {
    ...
    // 1. Handle National Teams
    if (clubId.startsWith("nat_") || (typeof clubInput === 'object' && clubInput.participantType === 'national')) {
      ...
      const firstNames = window.NATIONAL_NAMES[countryName]?.first || window.NATIONAL_NAMES["Germany"]?.first || ["Hans"];
      const lastNames = window.NATIONAL_NAMES[countryName]?.last || window.NATIONAL_NAMES["Germany"]?.last || ["Müller"];
    ...
    if (club && club.squad && club.squad.length > 0) return club.squad;

    // 3. Check window.leaguesEngine.clubSquads
    if (window.leaguesEngine?.clubSquads && window.leaguesEngine.clubSquads[clubId]) {
      return window.leaguesEngine.clubSquads[clubId];
    }
  ...
  _finishSim() {
    ...
    const { xpEarned, matchMoney } = window.userCareer.recordMatchPerformance(userGoals, assists, 80, rating, compKey);
