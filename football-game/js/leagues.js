...
class LeaguesEngine {
  ...
  getCulturalCountry(countryName) {
    ...
    if (['england', 'usa', 'canada', 'australia', 'new zealand', 'scotland', 'wales', 'ireland', 'jamaica', 'haiti'].some(eu => c.includes(eu))) {
      return 'England';
    }
    ...
    if (['croatia', 'poland', 'ukraine', 'georgia', 'albania', 'romania', 'greece'].some(eu => c.includes(eu))) {
      return 'Croatia';
    }
    // 6. African Culture
    if (['senegal', 'ghana', 'nigeria', 'ivory coast', 'cameroon', 'south africa', 'mali'].some(eu => c.includes(eu))) {
      return 'Senegal';
    }
  ...
  getSeasonsRemaining(year, compKey, region) {
    if (compKey === 'national_world_cup' || compKey === 'world_cup') {
      const diff = (year - 2030) % 4;
      if (diff === 0) return 0;
      return 4 - Math.abs(diff);
    }
    ...
    const diff = (year - startYear) % cycle;
    if (diff === 0) return 0;
    return cycle - Math.abs(diff);
  }
    ...
    Object.keys(this.clubSquads).forEach(clubId => {
      ...
      squad.forEach((pl, idx) => {
        ...
        if (pl.age > 39) {
          ...
          const firstNames = window.NATIONAL_NAMES[country]?.first || window.NATIONAL_NAMES["England"].first;
          const lastNames = window.NATIONAL_NAMES[country]?.last || window.NATIONAL_NAMES["England"].last;
  ...
  generateSquad(clubId, avgOvr) {
    ...
    const country = leagueToCountry[leagueId] || "Turkey";
    const countriesPool = this.nationalTeams.map(n => n.name);

    return positions.map((pos, idx) => {
      // 70% chance local player, 30% foreign international player!
      let playerCountry = country;
      if (idx !== 0 && Math.random() < 0.30) {
        playerCountry = countriesPool[Math.floor(Math.random() * countriesPool.length)];
      }
  ...
  getUclClubsPool(excludeClubId = null) {
    return this.getTopClubsPool(excludeClubId).filter(c => {
      ...
      return ['turkey', 'england', 'spain', 'italy', 'germany', 'france', 'netherlands', 'portugal'].some(eu => country.includes(eu));
    });
  }
  ...
  buildSeasonSchedule(profile = null) {
    ...
    if (europeanLeague && userClub) {
      ...
      Object.values(this.leagues).forEach(lg => {
        if (lg.tier === 2) {
          const country = String(lg.country || '').toLowerCase();
          const isEuropean = ['turkey', 'england', 'spain', 'italy', 'germany', 'france', 'netherlands', 'portugal'].some(eu => country.includes(eu));
          if (isEuropean) {
            lg.clubs.forEach(c => {
    ...
    if (profile?.isNationalTeamCalledUp && userNationality) {
      const year = profile?.stats?.season?.year || 2026;
  ...
  simulateGameweek(userClubId, userMatchScore = null, eventContext = null, userNationality = null) {
    ...
    if (currentFix.competitionKey !== 'league') {
      ...
      if (currentFix.competitionKey === 'national_final' || currentFix.stageLabel === 'Final') {
        const compName = currentFix.competitionName;
        if (won) {
          if (window.userCareer?.profile?.trophies) {
