...
class FootballApp {
  constructor() {
    ...
    console.log = (...args) => {
      ...
      this._addDebugLog("LOG", args.join(" "));
    };

    const oldErr = console.error;
    console.error = (...args) => {
      oldErr.apply(console, args);
      this._addDebugLog("ERROR", args.join(" "));
    };
  ...
  init() {
    // Hide the local security protocol warning overlay
    const corsWarning = document.getElementById('cors-warning-overlay');
  ...
  updatePreviewOvr() {
    ...
    const calculatedOvr = Math.min(maxCeiling, Math.max(40, Math.round(ovr)));
  ...
  submitPlayerCreation() {
    const nameInput = document.getElementById('create-name');
    const name = (nameInput && nameInput.value) ? nameInput.value : "Jack Sterling";
  ...
  _addDebugLog(type, msg) {
    if (!this._debugLogs) this._debugLogs = [];
    const timestamp = new Date().toLocaleTimeString();
    const color = type === 'ERROR' ? '#ef4444' : '#10b981';
    const cleanMsg = String(msg || '').replace(/</g, "&lt;").replace(/>/g, "&gt;");
    
    this._debugLogs.push(`[${timestamp}] <span style="color: ${color}; font-weight: bold;">[${type}]</span> ${cleanMsg}`);
    if (this._debugLogs.length > 100) this._debugLogs.shift();
    
    const panel = document.getElementById('debug-console-panel');
    if (panel) {
      panel.innerHTML = this._debugLogs.map(l => `<div style="border-bottom: 1px solid rgba(255,255,255,0.03); padding: 4px 0;">${l}</div>`).join('');
    ...
    if (statusText) {
      const errCount = this._debugLogs.filter(l => l.includes('[ERROR]')).length;
      if (errCount > 0) {
        statusText.innerHTML = `<span style="color: #ef4444; font-weight: bold;">⚠️ ${errCount} Error(s) Detected!</span>`;
        const drawer = document.getElementById('debug-console-drawer');
        if (drawer) drawer.style.borderTopColor = "#ef4444";
      } else {
        statusText.innerHTML = `<span style="color: #10b981;">Engine Running (0 errors)</span>`;
        const drawer = document.getElementById('debug-console-drawer');
        if (drawer) drawer.style.borderTopColor = "#10b981";
      }
    }
  }
  ...
  toggleDebugConsole() {
    const panel = document.getElementById('debug-console-panel');
    if (!panel) return;
    const isClosed = panel.style.height === '' || panel.style.height === '0px';
    panel.style.height = isClosed ? '220px' : '0px';
  ...
  clearDebugLogs() {
    ...
    const statusText = document.getElementById('debug-status-text');
    if (statusText) statusText.innerHTML = `<span style="color: #10b981;">Engine Running (0 errors)</span>`;
  }
}
