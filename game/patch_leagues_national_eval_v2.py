# patch_leagues_national_eval_v2.py
with open("js/leagues.js", "r", encoding="utf-8") as f:
    content = f.read()

# Let's find insertUclFinal() start and end index to append our new insertion methods!
idx = content.find("  insertUclFinal() {")
if idx == -1:
    print("Could not find insertUclFinal")
    exit(1)

# Let's find where the next method (processUclQualificationResult) starts
next_idx = content.find("  processUclQualificationResult(profile, points) {")
if next_idx == -1:
    print("Could not find processUclQualificationResult")
    exit(1)

# Replacement block for UCL/Cup/National insertion methods:
insertion_methods = """  insertUclFinal() {
  ...
  insertCupRound(roundKey, roundLabel) {
    ...
    // Select any non-user opponent club from standings
    let cupOpponent = this.standings.find(c => c.clubId !== userClubId);
    if (!cupOpponent) cupOpponent = this.standings[0];
    ...
    this.seasonSchedule.forEach(entry => {
      ...
      if (entry.competitionKey === 'league' && !entry.played) {
        leagueCount++;
        // Settle spacing:
        // For QF and SF: Insert after every 4 unplayed league matches!
    ...
    if (roundKey === 'final' || !cupInserted) {
      // Spliced/appended at the very end
      newSchedule.push(this.createScheduleEntry({
        id: `cup_${roundKey}`,
...
patched_content = content[:idx] + insertion_methods + content[next_idx:]

# Let's locate simulateGameweek and update it completely!
sim_start = patched_content.find("  simulateGameweek(userClubId, userMatchScore = null, eventContext = null, userNationality = null) {")
if sim_start == -1:
    print("Could not find start of simulateGameweek")
    exit(1)

sim_end = patched_content.find("  updateStandingsEntry(fix) {")
if sim_end == -1:
...
patched_content = patched_content[:sim_start] + new_sim_gameweek + patched_content[sim_end:]

with open("js/leagues.js", "w", encoding="utf-8") as f:
    f.write(patched_content)

print("Insertion methods and simulateGameweek successfully synced!")
